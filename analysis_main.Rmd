...
This procedure is slow, so alternatively, load pre-run data.
```{r}
# LOAD .RDATA HERE
```
...
ggsave('char_50_150.png', height = 7, width = 12)
```

Make a weighted Alaska-wide composite by averaging ecoregions.
```{r}
alaska_avg_100 <- compCHAR_100 %>% 
  filter(region != 'Alaska') %>% 
  group_by(yr.bp) %>%
  summarise_if(is.numeric, mean, na.rm = T)

alaska_avg_500 <- compCHAR_500 %>% 
  filter(region != 'Alaska') %>% 
  group_by(yr.bp) %>%
  summarise_if(is.numeric, mean, na.rm = T)

ggplot() +
  geom_line(data = alaska_avg_100, aes(x = yr.bp, y = composite.mean)) +
  geom_ribbon(data = alaska_avg_500, aes(x = yr.bp, ymin = CI.lower, ymax = CI.upper), alpha = 0.3) +
  geom_line(data = alaska_avg_500, aes(x = yr.bp, y = composite.mean), size = 1) +
  scale_x_reverse() +
  coord_cartesian(ylim = c(0,3.5)) +
  theme_bw() +
  labs(x = 'Time (cal years BP)', y = 'Biomass burning (standardized CHAR)')

ggsave('ak_as_avg.png', height = 7, width = 10)
```
...
# pctBurned_ak <- pctBurned_reg %>%
#   group_by(year) %>%
#   summarise(boot.median = mean(boot.median,na.rm =T),
#             boot.lower = mean(boot.lower,na.rm =T),
#             boot.upper = mean(boot.upper,na.rm =T))
```
...
<!--   theme_bw() + -->
<!--   scale_x_reverse(limits = c(8000,-60)) + -->
<!--   labs(x = 'Time (cal years BP)', y = 'CHAR:FRI (Fire severity)') -->
<!-- ``` -->
